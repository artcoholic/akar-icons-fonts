  ...
  .then(components => {
    ...
    return queueTasks(Object.values(components).map(component => () => {
      return get(component.image, {
        ...
        .then(response => {
          return fse.ensureDir(join(options.outputDir, options.format))
            .then(() => fse.writeFile(join(options.outputDir, options.format, `${component.name}.${options.format}`), response.body, (options.format === 'svg' ? 'utf8' : 'binary')))
        })
    }))
  })
  .catch(error => {
